...
//指定した日,施設の 空き状況(というか予約状況)
public String getReservationIn( String facility){
String res = "";
...
if(flagLogin ){
	Frame frame=null;
	//ログインダイアログの生成と表示
	ReservationCheck ld = new ReservationCheck(frame);
	ld.setVisible(true);
	ld.setModalityType(LoginDialog.ModalityType.APPLICATION_MODAL);	
	if ( ld.canceled){
		return "";
		}
	String reservation_userid = ld.tfUserID.getText();
	//パスワードはここでしか使わないので,ローカル変数に代入
	String password = ld.tfPassword.getText();	
//(1) MySQL を使用する準備
//connectDB();
...
try {
//予約情報を取得するクエリ
ResultSet rs = mysql.getReservationAt(reservation_userid);
boolean exist = false;
while(rs.next()){
...
ResultSet rs = mysql.selectReservation(rdate, facility);
//検索結果に対して重なりチェックの処理
boolean ng = true;	//重なりチェックの結果の初期値（重なっていない=true）を設定
//取得したレコード一つ一つに対して確認
while(rs.next()){
//レコードの開始時刻と終了時刻をそれぞれstartとendに設定
String start = rs.getString("start_time");
